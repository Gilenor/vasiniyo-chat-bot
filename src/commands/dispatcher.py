...
handlers = check_mods({
    ...
    "reply": {
        ...
        handle_stickers(sticker_to_sticker): handler(sticker_ok(sticker_to_sticker), content_types=["sticker"])
    },
    "captcha": {
        handle_new_user: handler(lambda m: bool(getattr(m, "new_chat_members", None)), content_types=["new_chat_members"]),
...
query_handlers = check_mods({
    "event": {
        handle_title_change_attempt: {"func": query_check_data("number_")}
    },
    "captcha": {
